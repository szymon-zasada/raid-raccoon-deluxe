...
Single-binary FreeBSD web UI for managing command execution, Samba users/shares, ZFS pools/datasets/snapshots, and cron-backed snapshot schedules. Uses Go stdlib `net/http` with embedded HTML/CSS/JS.

## Features
- Terminal page with SSE output streaming and allowlisted commands
- Samba user and share management with `testparm` + `service` reload
...
- Audit log with command + exit code

## Build
```sh
go build -o raidraccoon ./cmd/raidraccoon
```
Build produces a single executable with embedded UI assets.
...
- Creates the `raidraccoon` user/group
...
- Use `./raidraccoon passwd` to set the password hash + salt
- `allowed_cmds` is enforced for the Terminal page (absolute paths only)
- `zfs.allowed_prefixes` constrains all ZFS operations
- `binary_path` is used by the scheduler to call `raidraccoon snapshot`
- HTTP Basic Auth uses the config `auth.username`, `auth.salt_hex`, `auth.password_hash_hex`
...
```
The `[global]` section and any preamble lines are preserved, but share sections are rewritten based on the UI. Any `include = ...` lines are removed when saving so the app can fully own the share definitions. If you want a different path, set `samba.include_file` in the config (despite the name, it now refers to the full config file).
...
- Navigation loads without JS console errors; sidebar + menu links work
